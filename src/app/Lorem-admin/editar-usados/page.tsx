...
function buildUrl(page: number) {
  return `${API_BASE}/api/usados?page=${page}&size=${PAGE_SIZE}`
}
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="px-4 pt-3 pb-4 flex flex-col gap-2 flex-1">
        ...
        <div className="flex gap-2 text-[11px] text-slate-500 flex-wrap">
          ...
          <span className="bg-slate-50 border border-slate-100 px-2 py-0.5 rounded">{formatKm(vehicle.km)} km</span>
        ...
        {/* Edit button */}
        <Link
          href={`/Lorem-admin/editar-usados/${vehicle.id}`}
          className="block w-full mt-1 py-2 text-center text-[12px] font-semibold bg-fuchsia-600 hover:bg-fuchsia-700 text-white rounded-lg transition-colors"
        >
          Editar
        </Link>
      </div>
...
export default function EditarUsadosPage() {
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <header className="bg-[#1e3a5f] text-white px-6 py-3 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[15px] font-semibold leading-tight">Editar usados</p>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-end justify-between mb-6">
          <h1 className="text-[22px] font-semibold text-slate-800">Vehículos usados</h1>
                ...
                {Array.from({ length: totalPaginas }).map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 text-[12px] font-medium rounded-lg transition-colors ${
                      i === page ? 'bg-fuchsia-600 text-white' : 'text-slate-500 hover:bg-slate-100'
                    }`}
                  >
                    {i + 1}
                  </button>
